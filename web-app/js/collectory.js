...
function setNumbers(totalBiocacheRecords) {
    ...
    switch (totalBiocacheRecords) {
        case 0:
            recordsClause = "No records";
            break;
        case 1:
            recordsClause = "1 record";
            break;
        default:
            recordsClause = addCommas(totalBiocacheRecords) + " records";
    }
...
function initializeLocationMap(canBeMapped, lat, lng) {
    ...
    var marker;
    if (canBeMapped) {
        if (lat == undefined || lat == 0 || lat == -1) {
            lat = -35.294325779329654
        }
        if (lng == undefined || lng == 0 || lng == -1) {
                ...
                if (data.lastYear.events == '0') {
                    ...
                    for (var period in periods) {
                        ...
                        for (var reason in sortedReasons) {
                            stats += "<tr><td>" + capitaliseFirstLetter(sortedReasons[reason].key) + "</td><td style='text-align: right;'><span class='number'>" +
